...
export default function AdminDeveloppeurs() {
  ...
  useEffect(() => { fetchDeveloppeurs(); }, []);

  const fetchDeveloppeurs = async () => {
    setLoading(true);
    try {
      const res = await api.get("/developpeurs");
      setDeveloppeurs(res.data.data ?? []);
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
  ...
  const openEdit = (d) => {
    setNom(d.nom); setEditId(d.id); setError(""); setModal("edit");
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (modal === "add") {
        await api.post("/developpeurs", { nom });
      } else {
        await api.put(`/developpeurs/${editId}`, { nom });
      }
      setModal(null);
      fetchDeveloppeurs();
    } catch (err) {
